...
void start_server(int server_socket) {
    ...
    pthread_create(&keepalive_pid, NULL, (void*)thread_pool_keepalive,
                   (void*)pool);
    ...
    while (1) {
        ...
        int client_sock = accept(server_socket,
                                 (struct sockaddr*)&remote_client, &client_len);
        if (client_sock != -1) {
            struct SocketArg* arg =
                (struct SocketArg*)malloc(sizeof(struct SocketArg));
...
char* local_time() {
    ...
    sprintf(loc_time, "%04d-%02d-%02d %02d:%02d:%02d", 1900 + p->tm_year,
